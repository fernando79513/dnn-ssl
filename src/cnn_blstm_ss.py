...
if __name__ == "__main__":
    ...
    pmaps = np.load('data/matrix_voice/dev/pmap_0_src_clean_100.npy')
    stft_data = np.load('data/matrix_voice/dev/stft_data_0_src_clean_100.npy')
    print(pmaps.shape)
    print(stft_data.shape)
    for i in range (200,300,100):
        new_pmaps = np.load(f'data/matrix_voice/dev/pmap_0_src_clean_{i}.npy')
        new_stft = np.load(f'data/matrix_voice/dev/stft_data_0_src_clean_{i}.npy')
        pmaps = np.concatenate((pmaps, new_pmaps))
        stft_data = np.concatenate((stft_data, new_stft))
        print(f'concatenated 1_src_noise {i}')
    for i in range (100,300,100):
        new_pmaps = np.load(f'data/matrix_voice/dev/pmap_1_src_noise_{i}.npy')
        new_stft = np.load(f'data/matrix_voice/dev/stft_data_1_src_noise_{i}.npy')
        pmaps = np.concatenate((pmaps, new_pmaps))
        stft_data = np.concatenate((stft_data, new_stft))
    for i in range (200,1400,100):
    ...
    for i in range (100,1400,100):
        new_pmaps = np.load(f'data/matrix_voice/dev/pmap_2_src_clean_{i}.npy')
        new_stft = np.load(f'data/matrix_voice/dev/stft_data_2_src_clean_{i}.npy')
        pmaps = np.concatenate((pmaps, new_pmaps))
        stft_data = np.concatenate((stft_data, new_stft))
        print(f'concatenated 2_src_clean {i}')
    for i in range (100,500,100):
        new_pmaps = np.load(f'data/matrix_voice/dev/pmap_2_src_clean_{i}.npy')
        new_stft = np.load(f'data/matrix_voice/dev/stft_data_2_src_clean_{i}.npy')
        pmaps = np.concatenate((pmaps, new_pmaps))
    ...
    blstm.save("data/matrix_voice/models/blstm_pit.h5", overwrite=True)
